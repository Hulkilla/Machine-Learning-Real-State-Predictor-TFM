...
def plot_predictions(actual, predicted, title="Valores Reales vs Predichos"):
    ...
    plt.plot([min(actual), max(actual)], [min(actual), max(actual)], color='red', linestyle='--', label="Línea ideal (y=x)")
    ...
    plt.plot(actual, lower_bound, color='blue', linestyle='--', label="Límite inferior (70%)")
    plt.plot(actual, upper_bound, color='green', linestyle='--', label="Límite superior (130%)")
...
def evaluate_model(model_name, model, X_train, y_train, X_test, y_test, model_results, scaler, cv=5):
    """
    ...
    y_test_original = np.exp(scaler.inverse_transform(y_test.values.reshape(-1, 1)).ravel())
    predictions_original = np.exp(scaler.inverse_transform(predictions.reshape(-1, 1)).ravel())
    ...
    neg_mse_cv_scores = cross_val_score(model, X_train, y_train, cv=cv, scoring='neg_mean_squared_error')
    r2_cv_scores = cross_val_score(model, X_train, y_train, cv=cv, scoring='r2')

    # Calcular RMSE original desde MSE escalado
    rmse_cv_scores = np.sqrt(-neg_mse_cv_scores)
    rmse_cv_scores_original = scaler.inverse_transform(rmse_cv_scores.reshape(-1, 1)).ravel()
    ...
    avg_deviation_percentage = np.mean(np.abs((y_test_original - predictions_original) / (y_test_original + epsilon))) * 100

    # Buscar si ya existe el modelo en la lista de resultados
    existing_model_index = next((i for i, result in enumerate(model_results) if result['Modelo'] == model_name), None)
...
def get_model_results(model_name, model_results):
    ...
    result = next((item for item in model_results if item['Modelo'] == model_name), None)

    if result:
        # Convertir métricas numéricas a float si es necesario
        result = {key: (float(value) if isinstance(value, np.float64) else value) for key, value in result.items()}
        return result
    else:
...
def evaluate_feature_importance(model, feature_names, title="Importancia de las variables"):
    """
    ...
    importance_df = pd.DataFrame({
        ...
        '%IncMSE': feature_importances * 100,  # Simulación del impacto en el MSE como porcentaje
        'IncNodePurity': np.random.uniform(1e10, 1e12, len(feature_names))  # Placeholder para pureza de nodo
    })
    ...
    plt.bar(importance_df['Feature'], importance_df['%IncMSE'], color='steelblue')
...
def train_model(model, X_train, y_train, X_val, y_val, n_epochs, batch_size, loss_fn, optimizer, scaler, patience=10):
    """
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        with torch.no_grad():
            ...
            y_val_inv = np.exp(scaler.inverse_transform(y_val.cpu().numpy().reshape(-1, 1)).ravel())
            y_val_pred_inv = np.exp(scaler.inverse_transform(y_val_pred.cpu().numpy().reshape(-1, 1)).ravel())

            # 🔎 Métricas en escala original
            val_loss = loss_fn(y_val_pred, y_val).item()  # Se queda en log+escalado (no se transforma)
        ...
        if (epoch + 1) % 5 == 0:
            print(f"Epoch {epoch + 1}/{n_epochs} | Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | Val MAE: {val_mae:,.2f}")
        ...
        if val_loss < best_loss:
            ...
            if patience_counter >= patience:
                print(f"⏹️ Early stopping activado en la época {epoch}. Mejor Val Loss: {best_loss:.4f}")
                break

    model.load_state_dict(best_weights)
    return best_loss, train_loss_hist, val_loss_hist, val_mae_hist, val_deviation_hist
...
def calculate_percentage_deviation(y_real, y_pred):
    ...
    deviation_percentage = torch.mean(torch.abs((y_real - y_pred) / (y_real + epsilon))) * 100
